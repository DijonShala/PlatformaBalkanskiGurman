...
router.patch("/user/:id/password", auth, isOwnerOrAdmin(User), ctrlUsers.changePassword);
...
router.get("/restaurants/codelist/:codelist", ctrlRestaurants.listCodelistValues);
router.post("/restaurants", auth, upload.array("restaurant_photos", 5), ctrlRestaurants.createRestaurant);
router.put("/restaurants/:id", auth, isOwnerOrAdmin(Restaurant), upload.array('restaurant_photos'), ctrlRestaurants.updateRestaurant);
router.delete("/restaurants", auth, isAdmin, ctrlRestaurants.deleteAllRestaurants);
router.delete("/restaurants/:id", auth, isOwnerOrAdmin(Restaurant), ctrlRestaurants.deleteRestaurant);
/**
 * Review
 */
...
router.post("/restaurants/:idrest/reviews", auth, upload.single('review_photo'), ctrlReviews.postReview);
router.put("/restaurants/:idrest/reviews/:id", auth, isOwnerOrAdmin(Review), ctrlReviews.updateReview);
router.delete("/restaurants/:idrest/reviews/:id", auth, isOwnerOrAdmin(Review), ctrlReviews.deleteReview);

/**
 * Initial data (for testing)
 */
router.route("/db").post(auth, isAdmin, ctrlDataB.addInitialData).delete(auth, isAdmin, ctrlDataB.deleteData);
