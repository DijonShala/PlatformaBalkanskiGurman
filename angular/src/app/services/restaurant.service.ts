...
export class RestaurantService {
  ...
  getCodelistValues(
    ...
    return this.http
      .get<{
        status: string;
        data: string[];
      }>(`${this.apiUrl}/restaurants/codelist/${field}`)
      .pipe(map((res) => res.data));
  }
  ...
  postReview(restaurantId: number, data: FormData): Observable<Review> {
    ...
    return this.http
      .post<{ data: Review }>(url, data, { headers })
      .pipe(map((res) => new Review(res.data)));
  }
